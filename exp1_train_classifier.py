...
def main(config):
    ...
    if opt == 'lee':
        if config.get('train_dataset') == 'aircraft-fs' or config.get('train_dataset') == 'cub-cropped':
            ...
            optimizer = torch.optim.SGD([{'params': model.parameters()}], lr=0.1, momentum=0.9, weight_decay=5e-4, nesterov=True)
            lambda_epoch = lambda e: 1.0 if e < 60 else (0.06 if e < 90 else 0.012 if e < 120 else (0.0024))
            lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_epoch, last_epoch=-1)
    ...
    for epoch in range(cur_epoch, max_epoch + 1 + 1):
        ...
        if config.get('train_dataset'): class_iter = iter(train_loader)     
        if config.get('fs_dataset_train'): meta_iter = iter(fs_train_loader)
